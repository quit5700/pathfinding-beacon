...
- 下载页：[GitHub Releases（GitHub 发布页）](https://github.com/quit5700/pathfinding-beacon/releases)
- 每次新增 Minecraft（我的世界）适配版本后，都需要同时生成本地 JAR（Java 归档）文件并上传到 Releases（发布页）。

## 当前源码构建配置

- Minecraft（我的世界）Java 版：`1.21.11`
- Java（爪哇）：`21`
- Fabric Loader（Fabric 模组加载器）：`0.19.3`
- Fabric API（Fabric 应用程序接口）：`0.141.4+1.21.11`
- Fabric Loom（Fabric 构建插件）：`1.17.12`
- Yarn Mappings（Yarn 命名映射）：`1.21.11+build.6`
- Gradle Wrapper（Gradle 包装器）：`9.5.0`
